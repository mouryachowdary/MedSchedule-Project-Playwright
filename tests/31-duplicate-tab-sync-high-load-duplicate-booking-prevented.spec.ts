...
test('31a. [Active Date] duplicate booking is prevented even under high load or multiple parallel requests.', async ({ browser }) => {
  ...
  try {
    ...
    if (isFirefox) {
      for (let i = 0; i < users.length; i++) {
        await preparePatientForNextAvailableDate(users[i], parallelUsers[i]);
      }
    } else {
      await Promise.all(
        users.map((user, index) => preparePatientForNextAvailableDate(user, parallelUsers[index]))
      );
    }
...
test('31b. [Future Date] duplicate booking is prevented even under high load or multiple parallel requests.', async ({ browser }) => {
  test.setTimeout(getTestTimeout(90000, test.info().project.name));
