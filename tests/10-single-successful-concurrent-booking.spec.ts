...
test('10a. [Active Date] only one booking is successful when Users 007 & 008 simultaneously book the same time slot.', async ({ browser }) => {
  ...
  try {
    ...
    await Promise.all([
      preparePatientForCurrentDate(userA, patientA),
      preparePatientForCurrentDate(userB, patientB),
    ]);
...
test('10b. [Future Date] only one booking is successful when Users 007 & 008 simultaneously book the same time slot.', async ({ browser }) => {
