...
test('30a. [Active Date] retry mechanism allows User 008 to successfully book a different available slot after failure.', async ({ browser }) => {
  ...
  try {
    ...
    await expect
      .poll(async () => slotUnavailable(user008, conflictSlot), {
        timeout: user008.timeout(15000),
      })
      .toBeTruthy();
...
test('30b. [Future Date] retry mechanism allows User 008 to successfully book a different available slot after failure.', async ({ browser }) => {
  test.setTimeout(getTestTimeout(60000, test.info().project.name));
